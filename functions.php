...
if (!function_exists('luna_setup')):
    ...
    function luna_setup() {
        ...
        function luna_wpautop_nobr($content) {
            return wpautop($content, false);
        }

        add_filter('the_content', 'luna_wpautop_nobr');
        add_filter('the_excerpt', 'luna_wpautop_nobr');
...
add_action('wp_enqueue_scripts', function() {
    ...
    if ($post && (has_shortcode($post->post_content, 'image-slider') || has_shortcode($post->post_content, 'content-slider'))) {
	...
	echo '<a href="' . $homeLink . '">' . $home . '</a> ' . $delimiter . ' ';
        ...
        if (is_category()) {
            ...
            if ('talk' == get_post_type()) {
                echo $before . $options['label-talk-pl'] . $after;
...
// maybe wont work on multisite installations. please use plugins instead.
    ...
    function contentSlider($atts) {
        ...
        if ($the_query->have_posts()) :
            $output = "<div class=\"flexslider contentslider\">\n";
    ...
    function luna_imageSlider($atts, $content = null) {
        return "<div class=\"flexslider imageslider\">\n"
                . "<ul class=\"slides\">\n"
    ...
    add_shortcode('image-slider', 'luna_imageSlider');

    function luna_imageSliderSlides($atts, $content = null) {
        return "<li>\n"
                . do_shortcode($content)
                . "</li>\n";
    }

    add_shortcode('slide', 'luna_imageSliderSlides');
    ...
    /**
     * Render custom fields of custom post type 'talk'
     */
    function luna_talk_fields() {
        global $post;
        global $options;
        $post_id = $post->ID;
        ...
        $field_label = array(
            'talk_kurz' => $options['label-short'],
            'talk_datum' => 'Datum',
            'talk_beginn' => 'Zeit',
            'talk_raum' => 'Raum',
            'talk_talk_max-teilnehmer' => 'max. Teilnehmer',
            'talk_verfuegbar' => 'Verf&uuml;gbare Pl&auml;tze',
            '_selected_speaker' => $options['label-speaker'],
        );

        $str .= "<div>";
        if (isset($talk_kurz) && (strlen(trim($talk_kurz)) > 0)) {
            $str .= "<span class='talk-label kurz'>" . $options['label-short'] . "</span> <span class='talk-kurz'>" . $talk_kurz . "</a><br>";
        }
        if (isset($talk_referentname) && (strlen(trim($talk_referentname)) > 0)) {
            $str .= "<span class='talk-label speaker'>" . $options['label-speaker'] . ":</span> <span class='talk-speaker'><a href='" . $talk_referentlink . "'>" . $talk_referentname . "</a></span><br>";
        }
        if (isset($talk_datum) && (strlen(trim($talk_datum)) > 0)) {
            $str .= "<span class='talk-label date'>Datum:</span> <span class='talk-date' title='" . date('Y-m-d', strtotime($talk_datum)) . "'>" . $talk_datum . "</span><br>";
        }
        if (isset($talk_beginn) && (strlen(trim($talk_beginn)) > 0)) {
            $str .= "<span class='talk-label time'>Zeit:</span> <span class='talk-start dtstamp' title='" . $dtstamp_beginn . "'>" . $talk_beginn . "</span>";
            if (isset($talk_ende) && (strlen(trim($talk_ende)) > 0)) {
                $str .= " &ndash; <span class='talk-end dtstamp' title='" . $dtstamp_ende . "'>" . $talk_ende . "</span><br>";
            }
        }
        if (isset($talk_raum) && (strlen(trim($talk_raum)) > 0)) {
            $str .= "<span class='talk-label place'>Ort:</span> <span class='talk_ort'>" . $talk_raum . "</span><br>";
        }
        if (isset($talk_max_teilnehmer) && (strlen(trim($talk_max_teilnehmer)) > 0)) {
            $str .= "<span class='talk-label max-teilnehmer'>max. Teilnehmer:</span> <span class='talk_max_teilnehmer'>" . $talk_max_teilnehmer . "</span><br>";
        }
        if (isset($talk_verfuegbar) && (strlen(trim($talk_verfuegbar)) > 0)) {
            $str .= "<span class='talk-label verfuegbar'>noch verf&uuml;gbar:</span> <span class='talk_verfuegbar'>" . $talk_verfuegbar . "</span>";
            if (get_post_meta($post->ID, 'talk_warteliste', true) == "on") {
        ...
        if (isset($talk_aufzeichnung) && (strlen(trim($talk_aufzeichnung)) > 0)) {
            $str .= '<li class="video"><a href="' . $talk_aufzeichnung . '">Videoaufzeichnung</a></li>';
        }
        if (isset($talk_folien) && (strlen(trim($talk_folien)) > 0)) {
            $str .= '<li class="folien"><a href="' . $talk_folien . '">Vortragsfolien</a></li>';
        }
        $str .= '</ul>';
        echo $str;
    }
     ...
     */
    function luna_speaker_talk_list() {
        global $post;
        global $options;
        $str = "<h3 style='clear:both;'>" . $options['label-talk-pl'] . ":</h3>";
        $str .= "<ul>";
        $args = array('post_type' => 'talk', 'orderby' => 'title', 'order' => 'ASC');
        ...
        if (!empty($talks)) {
            foreach ($talks as $talk) {
                $custom_fields = get_post_meta($talk->ID);
    ...
    /**
     * Contact Form 7 : Shortcode [talk-list] aus Liste der Vortäge (= Custom Post Type 'talk') erstellen
     * z.B. [select* talk-list include_blank]
     */
    function luna_add_plugin_list_to_contact_form($tag, $unused) {
        ...
        foreach ($talks as $talk) {
            $verfuegbar = get_post_meta($talk->ID, 'talk_verfuegbar', true);
            $kurz = get_post_meta($talk->ID, 'talk_kurz', true);
            $datum = get_post_meta($talk->ID, 'talk_datum', true);
            $warteliste = get_post_meta($talk->ID, 'talk_warteliste', true);
            if ((isset($verfuegbar)) && ($verfuegbar > 0)) {
                $tag['raw_values'][] = $kurz . "_" . $talk->post_name;
                $tag['values'][] = $kurz . "_" . $talk->post_name;
                $tag['labels'][] = $datum . ": " . $talk->post_title . " (" . $kurz . ")";
            } elseif ((isset($verfuegbar)) && ($verfuegbar == 0) && $warteliste == "on") {
                $tag['raw_values'][] = $kurz . "_" . $talk->post_name . "_warteliste";
                $tag['values'][] = $kurz . "_" . $talk->post_name . "_warteliste";
                $tag['labels'][] = $datum . ": " . $talk->post_title . " (" . $kurz . ") &ndash; " . _x('WAITING LIST', 'Addition to select list option on registration form', 'luna');
            }
        }
    ...
    function luna_contextual_help() {
        $helptexts = array(
            array(
                'id' => 'accordion',
                'title' => __('Accordion', 'luna'),
                'content' => '<h3>Accordion einbinden</h3>'
                . '<p>Ein Accordion (aufklappbare Textbereiche) können Sie über folgenden Shortcode einbinden:</p>'
                . '<code>[collapsibles]</code><br />'
                . '<code>[collapse title="Erste Überschrift" color="blau"]</code><br />'
                . 'Erster Text Text Text...<br />'
                . '<code>[/collapse]</code><br />'
                . '<code>[collapse title="Zweite Überschrift" color="hellblau"]</code><br />'
                . 'Zweiter Text Text Text...<br />'
                . '<code>[/collapse]</code><br />'
                . '<code>[collapse title="Dritte Überschrift" color="gruen"]</code><br />'
                . 'Dritter Text Text Text...<br />'
                . '<code>[/collapse]</code><br />'
                . '<code>[/collapsibles]</code>'
                . '<p>Wichtig sind dabei die öffnenden und schließenden Shortcodes <code>[collapse]</code> und <code>[/collapse]</code> jeweils um die einzelnen Textabschnitte, sowie der öffnende und schließende Shortcode <code>[collapsibles]</code> und <code>[/collapsibles]</code>einmal um den gesamten Accordion-Block herum.</p>'
                . '<p>Folgende Farben stehen zur Verfügung: blau, hellblau, gruen und gelb. Die Farbangabe ist optional. Der Standardwert, wenn keine Farbe explizit angegeben wird, ist blau.</p>'
            ),
            array(
                'id' => 'slider',
                'title' => __('Slider', 'luna'),
                'content' => '<h3>Image-Slider</h3>'
                . '<p>Der Image-Slider zeigt eine Bildergalerie als Slideshow an.<br />'
                . 'Den Slider binden Sie mithilfe des folgenden Shortcodes ein:<br />'
                . '<code>[image-slider]</code><br />'
                . '<code>[slide]</code><br />'
                . 'Bild1<br />'
                . '<code>[/slide]</code><br />'
                . '<code>[slide]</code><br />'
                ...
                . '<code>[/slide]</code><br />'
                . '<code>[/image-slider]</code><br />'
                . '<p>Wichtig sind dabei die öffnenden und schließenden Shortcodes <code>[slide]</code> und <code>[/slide]</code> jeweils um die einzelnen Bilder, sowie der öffnende und schließende Shortcode <code>[image-slider]</code> und <code>[/image-slider]</code>einmal um den gesamten Slider-Block herum.</p>'
                . '<p>Die Bilder fügen Sie einfach mit der Funktion "Dateien hinzufügen" des Editors ein. Dabei wird der Beschreibungstext automatisch als Bildbeschreibung im Slider angezeigt.</p>'
                .'<h3>Content-Slider</h3>'
                . '<p>Mit dem Content-Slider können Sie Inhalte des Webauftritts in einem Slider durchlaufen lassen.</p>'
                . '<p>Den Slider binden Sie mithilfe des Shortcodes <code>[content-slider type=talk anzahl=6]</code>ein. <br />'
                . '<code>type</code> steht für den Inhaltstyp: Beitrag (article), Veranstaltung (talk) oder Referent (speaker).<br />'
                . 'Mit <code>anzahl</code> können Sie die Anzahl der durchlaufenden Inhalte begrenzen.</p>'
            ),
            array(
                'id' => 'veranstaltungen',
                'title' => __('Talks', 'luna'),
